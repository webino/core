 ...
 *
 * Add controller plugins table to profiler log.
 *
 * @category   Webino
 * @package    Core
 * @subpackage ProfilerPlugin
 ...
 * @link       http://pear.webino.org/core/
 */
class Webino_ProfilerPlugin_ControllerPlugins
    implements Webino_ProfilerPlugin_Interface
{
     ...
     * 
     * @return Webino_ProfilerPlugin_ControllerPlugins
     */
    ...
    {
        $plugins = $this->_frontController->getPlugins();

        $pluginsTable = array(array('stackIndex', 'class'));

        foreach ($plugins as $stackIndex => $object) {
            $pluginsTable[] = array($stackIndex, get_class($object));
        }

        $profiler->write(
            'send', $pluginsTable, sprintf(
                'Controller Plugins (%s):', count($plugins)
            ), 'TABLE'
        );
    }
}
